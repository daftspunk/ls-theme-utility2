...
<? elseif ($item->control_type == 'checkbox'): ?>
    ...
    <ul>
        <? foreach ($item->item_products as $item_product): ?>
            ...
            <li>
                <label>
                    <input class="bundle_product_selector" type="checkbox" 
                        name="<?= Shop_BundleHelper::get_product_selector_name($item, $item_product) ?>" 
                        value="<?= Shop_BundleHelper::get_product_selector_value($item_product) ?>" <?= checkbox_state($is_selected) ?> />
                    <?= h($item_product->product->name) ?>
                </label>
            ...
            <li>
                <label>
                    <input class="bundle_product_selector" type="radio" 
                        ...
                        value="<?= Shop_BundleHelper::get_product_selector_value($item_product) ?>" <?= radio_state($is_selected) ?>/>
                    <?= h($item_product->product->name) ?>
                </label>
