<table class="data_table products">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="right"><?= $item->bundle_master_order_item_id ? $item->get_bundle_item_quantity() : $item->quantity ?></td>
      <td class="right last total">
      ...
      ?>
          <tr class="bundle_totals">
            ...
            <td class="right"><?= format_currency($master_item->get_bundle_single_price()) ?></td>
            <td class="right"><?= format_currency($master_item->get_bundle_discount()) ?></td>
            <td class="right"><?= $master_item->quantity ?></td>
            <td class="right last"><?= format_currency($master_item->get_bundle_total_price()) ?></td>
          </tr>
      <? 
          endif;
        endif 
  ...
  <tfoot>
    <tr>
      ...
      <th class="right total"><?= format_currency($order->subtotal) ?></th>
    </tr>
    <? foreach ($order->list_item_taxes() as $tax): ?>
      <tr>
        <th class="right" colspan="4">Sales tax (<?= ($tax->name) ?>): </th>
        <th class="right total"><?= format_currency($tax->total) ?></th>
      </tr>
    <? endforeach ?>  
    <tr>
      <th class="right" colspan="4">Shipping</th>
      <th class="right total"><?= format_currency($order->shipping_quote) ?></th>
    </tr>
    <? foreach ($order->list_shipping_taxes() as $tax): ?>
      <tr>
        <th class="right" colspan="4">Shipping tax (<?= ($tax->name) ?>): </th>
        <th class="right total"><?= format_currency($tax->total) ?></th>
      </tr>
    <? endforeach ?>  
    <tr>
      <th class="right" colspan="4">Total</th>
      <th class="right"><span class="product_price"><?= format_currency($order->total) ?></span></th>
    </tr>
  </tfoot>
</table>​