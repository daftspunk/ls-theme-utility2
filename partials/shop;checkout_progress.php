...
<div class="checkout_progress clearfix">
  <ul class="clearfix">
  <?
    $current_found = false;
    $index = 0;
    ...
    <li <? if (!$current_found || $is_current): ?>class="active"<? endif ?>>
      ...
      <span class="name">
        <? if ($current_found): ?>
          <?= $name ?>
        <? else: ?>
          <a href="#" onclick="return $(this).getForm().sendRequest('on_action', {update:{'checkout_page': 'shop:checkout_partial'}, extraFields: {'move_to': '<?= $step ?>'}})"><?= h($name) ?></a>
        <? endif ?>
      </span>
    </li>
  <?
